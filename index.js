...
conn.on('user-presence-update', json => console.log(json.id + ' presence is => ' + json.type)) || console.log('Bot by ig:@raphser')
conn.on('message-status-update', json =>
{
   const participant = json.participant ? ' (' + json.participant + ')' : '' // participant exists when the message is from a group
   console.log(`[ ${moment().format("HH:mm:ss")} ] => bot by ig:@raphser`)
})
...
************************************
   ...
   {
    var items = ["ullzang girl", "cewe cantik", "hijab cantik", "korean girl", "remaja cantik", "cewek korea", "cewek jepang", "anselma", "tamara"];
   ...
   {
    var items = ["cowo ganteng", "cogan", "korean boy", "chinese boy", "japan boy", "cowok indo ganteng", "cowok korea", "jason bubu"];
...
if (text.includes("#animepict"))
   {
    var items = ["anime girl", "anime cantik", "anime", "anime aesthetic", "anime hd", "gambar anime hd", "loli"];
...
aruga.onGlobalParicipantsChanged(async (event) => {
        ...
        if (event.action === 'add' && event.who !== host) {
            await aruga.sendTextWithMentions(event.chat, `Halo, semoga ga betah hehe @${event.who.replace('@c.us', '')} \n\nHave fun with us`)
        }
        // kondisi ketika seseorang dikick/keluar dari group
        if (event.action === 'remove' && event.who !== host) {
            await aruga.sendTextWithMentions(event.chat, `Gudbai bro @${event.who.replace('@c.us', '')}, We'll miss you`)
        }
    })

    aruga.onIncomingCall(async (callData) => {
        // ketika seseorang menelpon nomor bot akan mengirim pesan
        await aruga.sendText(callData.peerJid, 'Dibilang jangan Nelpon! blocking..\n\n-bot')
        .then(async () => {
